...
checkpid() {
    MY_UID=`${ID} -u`
    # The '\$' is needed in the pgrep pattern to select 2bmbPG1, but not 2bmbPG1.sh
...
#	    ${ECHO} "testing the CWD commands"
#	    ${ECHO} ${BIN_CWD}
#	    ${ECHO} ${IOC_CWD}

	    if [ $BIN_CWD = $IOC_CWD ] ; then
#		${ECHO} " In the loop where the ioc is running"
...
start() {
	...
	screenpid
        ...
        cd ${IOC_STARTUP_DIR}
	# Run 2bmbPG1 inside a screen session
...
run() {
	...
	screenpid
        ...
        ${ECHO} "Starting ${IOC_NAME}"
        cd ${IOC_STARTUP_DIR}
	# Run 2bmbPG1 outside of a screen session, which is helpful for debugging
	${IOC_CMD}
    fi
}
...
launch_medm() {
${ECHO}
    /local/epics/synApps_6_0/support/areaDetector-R3-3-2/ADSpinnaker/iocs/spinnakerIOC/iocBoot/iocSpinnaker/start_caqtdm

}

usage() {
    ${ECHO} "Usage: $(${BASENAME} ${SNAME}) {start|stop|restart|status|console|run|medm}"
}
